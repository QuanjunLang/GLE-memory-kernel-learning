function [basis_mat, knots, N] = spline_basis_new(lb, rb, knot_num, deg, basis_derivative, xgrid, free_bdry)

% Using the default MATLAB function to generate thet B-spline basis
% The result is a function handle, can be used with numerical integrator
% Input: [lb, rb] is the domain for generating the spline functions
%        knot_num: number of knots
%        deg: degree of the polynomials +1 .
%        free_bdry: If true, the boundary value has freedom. Otherwise
%                   boundary value is set to be 0.


%%
k = deg+1;

if free_bdry
    knots = augknt(linspace(lb, rb, knot_num+1),k);
else
    knots = linspace(lb, rb, knot_num+1);
end


colmat = spcol(knots, k, brk2knt(xgrid, basis_derivative));
[~, N] = size(colmat);

basis_mat = cell(basis_derivative, 1);
for i = 1:basis_derivative
    basis_mat{i} = colmat(i:basis_derivative:end, :);
end

% Recover the function handle from the coefficients
% sp = spmak(knots, coef);
% f = @(x) fnval(sp, x);
end



%% debug
% lb = -4;
% rb = 2;
% knot_num = 10;
% deg = 4;
%
% coef(1) = 1;
% sp = spmak(knots, coef);
% fnbrk(sp)
% g1 = @(x) fnval(sp, x);
%
% coef = zeros(knot_num+deg-1, 1)';
% coef(2) = 1;
% sp = spmak(knots, coef);
% fnbrk(sp)
% g2 = @(x) fnval(sp, x);
%
% fplot(g1, [lb, rb]);hold on;fplot(g2, [lb, rb]);




