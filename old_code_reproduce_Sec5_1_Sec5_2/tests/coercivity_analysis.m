...
for i = 1:length(omega_seq)
    ...
    [m_h_eps_seq(i), M_h_eps_seq(i)] = get_m_M(omega_seq(i), h_true_lap);
    ...
    hgrid_regression = h_true(xgrid_regression);
    ...
    loss_est_regression_seq(i) = sum((h_conv_theta_dx + dg_grid_regression).^2.*rho_grid_regression)*dx_regression;
    loss_est_prony_seq(i) = sum((h_conv_theta_prony_dx + dg_grid_regression).^2.*rho_grid_regression)*dx_regression;
end
