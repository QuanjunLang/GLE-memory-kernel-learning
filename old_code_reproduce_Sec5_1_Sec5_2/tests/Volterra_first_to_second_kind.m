...
figure;
subplot(131);hold on;
plot(I.xgrid, h1(I.xgrid));
plot(I.xgrid, h(I.xgrid));
...
%% Change to Volterra equation of the second type
K2 = zeros(N, N);
for i = 1:N
    for j = 1:i
        K2(i, j) = g((i-j)*dx)*dx;
...
plot(xgrid, c_second, 'LineWidth', 3)
legend('LS','RKHS','Prony','true', 'reg Volterra', 'Volterra_second')
...
plot(log10(xgrid), log10(c_second), 'LineWidth', 3)
legend('LS','RKHS','Prony','true', 'reg Volterra', 'Volterra_second')
