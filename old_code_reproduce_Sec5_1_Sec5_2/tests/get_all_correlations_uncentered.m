function corr_func = get_all_correlations_uncentered(v, dt, F)
...
M = length(v);
corr_func.h = xcorr(v, v, 'unbiased')';
corr_func.h = corr_func.h(M:end);

corr_func.dh = xcorr(gradient(v, dt), v, 'unbiased')';
corr_func.dh = corr_func.dh(M:end);

corr_func.ddh = xcorr(4*del2(v, dt), v, 'unbiased')';
corr_func.ddh = corr_func.ddh(M:end);

corr_func.f = xcorr(F(v), v, 'unbiased')';
corr_func.f = corr_func.f(M:end);

corr_func.df = xcorr(gradient(F(v), dt), v, 'unbiased')';
