...
% omega = 0.0000001;
% omega = 0.1;
tau_grid_real = 10.^linspace(-3, 3, 100);
tau_grid = omega + 1i*tau_grid_real;

% gamma_z = abs(R_lap(tau_grid));
% z_gamma_z = abs(tau_grid.*R_lap(tau_grid));
z_gamma_z = zeros(size(tau_grid));
gamma_z = zeros(size(tau_grid));
for i = 1:length(z_gamma_z)
    z_gamma_z(i) = abs(tau_grid(i).*R_lap(tau_grid(i)));
    gamma_z(i) = abs(R_lap(tau_grid(i)));
end
...
m_h_eps_z_comb
upbd_iprv = sqrt((2/m_h_eps_z_comb)*(M_gamma*h_error_paper + g_error_paper));
